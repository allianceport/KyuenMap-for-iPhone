...
// in simulator we redefine to format for mattan Developer console
...
{\
	NSString * exceptionName = [@"org.mattan." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.mattan";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UImattanNativeItemNone = -1, 
	UImattanNativeItemSpinner = -2,
	UImattanNativeItemProgressBar = -3,
	
	UImattanNativeItemSlider = -4,
	UImattanNativeItemSwitch = -5,
	UImattanNativeItemMultiButton = -6,
	UImattanNativeItemSegmented = -7,
	
	UImattanNativeItemTextView = -8,
	UImattanNativeItemTextField = -9,
	UImattanNativeItemSearchBar = -10,
	
	UImattanNativeItemPicker = -11,
	UImattanNativeItemDatePicker = -12,
	
	UImattanNativeItemInfoLight = -13,
	UImattanNativeItemInfoDark = -14,
	
	UImattanNativeItemDisclosure = -15,
	
	UImattanNativeItemContactAdd = -16
};
